...
- Execute code safely in the browser for Java Script or at server 
side for Python (let's assume that other languages not supported yet, For security reasons, we don't want to execute code directly on the server. Instead, let's use WASM to execute the code only in the browser.)
